...
## Reference Source Plugins (`llmwiki/refs/`)
...
Drop a `.py` file in `llmwiki/refs/`:

```python
# llmwiki/refs/arxiv.py
PLUGIN_ID = "arxiv"
...
**Auto-discovery:** Any `.py` file in `llmwiki/refs/` with a `PLUGIN_ID` is automatically registered. No configuration needed.
...
Data source plugins handle ingestion. They live in `llmwiki/` as standalone modules.

### Structure

```python
# llmwiki/my_source.py
...
In `llmwiki/worker.py`, add to `_task_learn()`:
...
In `llmwiki/cli.py`, add ingest subcommands.
