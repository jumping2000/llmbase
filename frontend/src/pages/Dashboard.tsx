...
export function Dashboard() {
  ...
  const compileCard = (() => {
    ...
    if (compileStatus.status === 'completed') {
      ...
      return {
        icon: 'check_circle',
        title: t('dashboard.compile.completed'),
        ...
        body: t('dashboard.compile.completedBody', { count }),
      };
    }
    if (compileStatus.status === 'failed') {
      return {
        icon: 'error',
        title: t('dashboard.compile.failed'),
        detail: compileStatus.finished_at ? timeAgo(compileStatus.finished_at) : '',
        tone: 'text-rose-700 dark:text-rose-300 bg-rose-500/10 border-rose-500/20',
        body: compileStatus.error || t('dashboard.compile.unknownError'),
      };
    }
    return {
      icon: 'schedule',
      title: t('dashboard.compile.idle'),
      detail: '',
      tone: 'text-on-surface-variant bg-surface-high border-outline-variant/20',
      body: t('dashboard.compile.idleBody'),
    };
  })();
  ...
  return (
    <div className="p-8 max-w-[1100px] mx-auto">
        ...
        <div className="p-6 pb-4">
          ...
          {generating ? (
            ...
            <div className="text-sm text-on-surface-variant italic py-4">
              {stats?.article_count
                ? t('dashboard.xiciHint')
                : t('dashboard.xiciEmpty')}
            </div>
          )}
          ...
          {xici?.themes && xici.themes.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {xici.themes.map(theme => (
                <span key={theme} className="px-2.5 py-0.5 text-[11px] bg-primary/10 text-primary rounded-full">
                  {theme}
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { icon: 'description', label: t('dashboard.stat.rawDocs'), value: stats?.raw_count, color: 'text-secondary' },
          { icon: 'article', label: t('dashboard.stat.wikiArticles'), value: stats?.article_count, color: 'text-primary' },
          { icon: 'link', label: t('dashboard.stat.links'), value: stats?.link_count, color: 'text-tertiary' },
          { icon: 'health_and_safety', label: t('dashboard.stat.healthScore'),
      ...
      <div className="flex flex-wrap gap-3 mb-8">
          <button onClick={() => navigate('/qa')} className="flex items-center gap-2 px-5 py-3 bg-primary/10 border border-primary/20 rounded-xl text-sm hover:bg-primary/20 transition-colors">
            <Icon name="forum" className="text-primary text-[18px]" /> {t('dashboard.action.ask')}
          </button>
          <button onClick={() => navigate('/ingest')} className="flex items-center gap-2 px-5 py-3 bg-surface-container border border-outline-variant/30 rounded-xl text-sm hover:border-secondary/50 transition-colors">
            <Icon name="add_link" className="text-secondary text-[18px]" /> {t('dashboard.action.ingest')}
          </button>
          <button onClick={() => navigate('/health')} className="flex items-center gap-2 px-5 py-3 bg-surface-container border border-outline-variant/30 rounded-xl text-sm hover:border-tertiary/50 transition-colors">
            <Icon name="health_and_safety" className="text-tertiary text-[18px]" /> {t('dashboard.action.health')}
          </button>
      </div> */}
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <span className="text-[11px] text-outline">
              {llmUsage?.record_count ? t('dashboard.attempts', { count: llmUsage.record_count }) : t('dashboard.noData')}
            </span>
          </div>
        </div>
            ...
            <div className="rounded-xl border border-outline-variant/20 bg-surface-high p-4">
              <div className="flex items-end justify-between gap-4 mb-3">
                ...
                <div className="text-right text-xs text-on-surface-variant">
                  <div>{t('dashboard.successful')}: {usageSuccessRate}%</div>
                  <div>{t('dashboard.retryFallback')}: {usageRetryTokens.toLocaleString()}</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                <div className="rounded-lg bg-surface-container px-3 py-2 border border-outline-variant/20">
                  <div className="text-on-surface-variant uppercase tracking-wide mb-1">{t('dashboard.prompt')}</div>
                  <div className="font-medium text-on-surface">{llmUsage.totals.prompt_tokens.toLocaleString()}</div>
                </div>
                <div className="rounded-lg bg-surface-container px-3 py-2 border border-outline-variant/20">
                  <div className="text-on-surface-variant uppercase tracking-wide mb-1">{t('dashboard.output')}</div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-on-surface-variant">{t('dashboard.topModel')}</span>
              ...
              {llmUsage.malformed_record_count > 0 && (
                <p className="mt-3 text-[11px] text-amber-700 dark:text-amber-300">
                  {t('dashboard.malformedRecords', { count: llmUsage.malformed_record_count })}
                </p>
              )}

              {llmUsage.skipped_timestamp_count > 0 && llmUsage.applied_window !== 'all' && (
                <p className="mt-2 text-[11px] text-amber-700 dark:text-amber-300">
                  {t('dashboard.skippedTimestamps', { count: llmUsage.skipped_timestamp_count })}
                </p>
              )}
            </div>
            ...
            <div className="rounded-xl border border-outline-variant/20 bg-surface-high p-4">
              ...
              {lastLlmRequest ? (
                <>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[11px] ${lastLlmRequest.success ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'bg-rose-500/10 text-rose-700 dark:text-rose-300'}`}>
                      {lastLlmRequest.success ? t('dashboard.statusOk') : t('dashboard.statusFailed')}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                    <div className="rounded-lg bg-surface-container px-3 py-2 border border-outline-variant/20">
                      <div className="text-on-surface-variant uppercase tracking-wide mb-1">{t('dashboard.totalTokens')}</div>
                      <div className="font-medium text-on-surface">{lastLlmRequest.total_tokens.toLocaleString()}</div>
                    </div>
                    <div className="rounded-lg bg-surface-container px-3 py-2 border border-outline-variant/20">
                      <div className="text-on-surface-variant uppercase tracking-wide mb-1">{t('dashboard.attemptsLabel')}</div>
                ...
                <div className="py-4 text-sm text-on-surface-variant italic">
                  {usageWindow === 'all'
                    ? t('dashboard.noLlmCalls')
                    : t('dashboard.noLlmCallsInWindow')}
                </div>
              )}
            </div>
          </div>
